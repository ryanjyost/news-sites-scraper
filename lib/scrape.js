...
module.exports = async function(page, site, source) {
  ...
  switch (site.name) {
    ...
    case "bloomberg":
      if (source) {
        [$err, $linkObjects] = await to(
          page.$$eval(".story-package-module__story__headline-link", links => {
            let top10 = links.slice(0, 10);
        ...
        [$err, $linkObjects] = await to(
          page.evaluate(() => {
            const items = Array.from(
              document.querySelectorAll("#stream-panel li")
            );
    ...
    case "bbcnews":
      if (source) {
            ...
            $promiseForData = new Promise((resolve, reject) => {
              ...
              for (let item of items) {
                if ($array.length >= 10) {
                  ...
                  const title = item.querySelector(".media__title");
                  const link = item.querySelector(".media__link");
                  // const description = item.querySelector(".media__summary");
        ...
        [$err, $linkObjects] = await to(
          page.evaluate(() => {
            const titles = Array.from(
              document.querySelectorAll(".media__link")
            );
