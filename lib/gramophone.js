...
var stopWords = require("./stopWordsForExtraction.js");
...
exports.extract = function(text, options) {
  ...
  var stem = function(word) {
    ...
    if (
      !unstemmed.hasOwnProperty(stem) ||
      word.length < unstemmed[stem].length
    ) {
      unstemmed[stem] = word;
    }
    return stem;
  ...
  combined = Object.keys(combined).map(key => ({ [key]: combined[key] }));
  // .sortBy(_.last)
  // .reverse()
...
function blacklisted(term, extraStopWords) {
  ...
  return (
    _.indexOf(stopWords, term) !== -1 || _.indexOf(extraStopWords, term) !== -1
  );
}

function usePhrase(phrase, options) {
  return (
    whitelisted(phrase, options.startWords) ||
